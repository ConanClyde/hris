...
<div class="space-y-6">
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-neutral-800/50 transition-colors cursor-pointer" onclick="openViewModal({{ json_encode($noticeData) }})">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium" onclick="event.stopPropagation()">
                            <div class="flex justify-end gap-3 items-center">
                                ...
                                <button type="button" onclick="openDeleteModal('{{ route('admin.notices.destroy', $notice->id) }}', {{ json_encode($notice->title) }})" class="text-gray-400 hover:text-red-600 dark:hover:text-red-400 transition-colors" title="Delete">
    ...
    <div id="create-notice-modal" class="fixed inset-0 z-50 hidden" aria-modal="true" role="dialog">
        ...
        <div class="fixed inset-0 flex items-center justify-center p-4 pointer-events-none">
            <form action="{{ route('admin.notices.store') }}" method="POST" class="relative w-full max-w-2xl flex flex-col rounded-md border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 dark:ring-1 dark:ring-neutral-800 shadow-xl dark:shadow-2xl dark:shadow-black/40 pointer-events-auto">
                @csrf
...
@push('scripts')
<script>
    const noticesUpdateBase = @json(route('admin.notices.index'));

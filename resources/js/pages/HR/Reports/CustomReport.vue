...
const selectedSource = ref<string>(Object.keys(props.sources)[0] ?? 'employees');
...
const currentSourceColumns = computed(() => props.sources[selectedSource.value]?.columns ?? []);
        ...
        <div class="mx-auto w-full max-w-4xl space-y-6 p-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Select a data source, choose columns, and generate a CSV export.
                </p>
            </div>
            ...
            <Card class="border border-gray-200 dark:border-neutral-800">
                <CardHeader>
                    <CardTitle class="text-base">1. Select Data Source</CardTitle>
                </CardHeader>
                <CardContent>
            ...
            <Card class="border border-gray-200 dark:border-neutral-800">
                <CardHeader class="flex flex-row items-center justify-between">
                    ...
                    <div class="flex gap-2">
                        <button class="text-xs text-brand hover:underline dark:text-brand-light" @click="selectAll">
                            Select All
                        </button>
                        <button class="text-xs text-gray-500 hover:underline dark:text-gray-400" @click="deselectAll">
                            Clear
                        </button>
                    </div>
                </CardHeader>
                    ...
                    <div class="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                        ...
                        >
                            <div
                                ...
                                :class="selectedColumns.includes(col) ? 'border-brand bg-brand text-white' : 'border-gray-300 dark:border-neutral-600'"
                            >
                                <svg v-if="selectedColumns.includes(col)" class="size-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2">
                                    <path d="M2 6l3 3 5-5" />
                                </svg>
                            </div>
                    ...
                    <div class="flex items-center justify-between">
                        <p class="text-sm text-gray-500 dark:text-gray-400">
                            <template v-if="canExport">
                                Ready to export <strong>{{ selectedColumns.length }}</strong> column(s) from
                                <strong>{{ sources[selectedSource]?.label }}</strong>.
                            </template>
                            <template v-else>

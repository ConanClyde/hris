...
const noticeType = ref(props.notice.type || 'info');
const isActive = ref(props.notice.is_active ?? true);

watch(
    () => props.notice,
    (n) => {
        noticeType.value = n.type || 'info';
        isActive.value = n.is_active ?? true;
    },
    { immediate: true }
);

function expiresAtValue(): string {
    const v = props.notice.expires_at;
    if (!v) return '';
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="mx-auto w-full max-w-7xl space-y-4 p-4">
            ...
            <Form
                :action="hr.notices.update.url(notice.id)"
                method="post"

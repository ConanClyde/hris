        ...
        <div class="mx-auto w-full max-w-5xl space-y-6 p-4">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Track onboarding progress for each employee with standardized checklists.
                </p>
            </div>
                    ...
                    >
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p v-if="emp.date_hired" class="text-xs text-gray-500 dark:text-gray-400">
                                    Hired: {{ emp.date_hired }}
                                </p>
                            </div>
                        </div>
                        <div class="flex items-center gap-4">
                            <div v-if="emp.total > 0" class="flex items-center gap-2">
                                ...
                                <span class="text-xs text-gray-500">{{ emp.completed }}/{{ emp.total }}</span>
                            </div>
                            <Button v-else variant="outline" size="sm" @click.stop="initChecklist(emp.id)">
                                Initialize
                            </Button>
                        </div>
                    </div>

                    <div v-if="expandedId === emp.id && emp.checklist.length > 0" class="border-t border-gray-200 p-4 dark:border-neutral-700">
                        <div class="space-y-2">
                            <div
                            ...
                            >
                                <button
                                    ...
                                    :class="item.is_completed ? 'border-emerald-500 bg-emerald-500 text-white' : 'border-gray-300 dark:border-neutral-600'"
                                    @click="toggleItem(item.id)"
                                >
                                    <svg v-if="item.is_completed" class="size-3" viewBox="0 0 12 12" fill="none" stroke="currentColor" stroke-width="2">
                                        <path d="M2 6l3 3 5-5" />
                                    </svg>
                                </button>
                                <span
                                    class="flex-1 text-sm"
                                    :class="item.is_completed ? 'text-gray-400 line-through' : 'text-gray-900 dark:text-gray-100'"
                                >
                                    {{ item.title }}
                                </span>
                                <span
                                    class="inline-flex rounded-full px-2 py-0.5 text-[10px] font-medium capitalize"
                                    :class="categoryColors[item.category] ?? categoryColors.general"
                                >
                                    {{ item.category }}
                                </span>
                                <span v-if="item.completed_at" class="text-[10px] text-gray-400">{{ item.completed_at }}</span>
                            </div>
                        </div>
                    </div>
                </div>

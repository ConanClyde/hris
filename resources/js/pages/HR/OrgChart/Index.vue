...
const selectedSection = ref<{ id: number; name: string; divisionId: number; subdivisionId?: number } | null>(null);
...
const sectionEmployees = computed(() => {
    ...
    return props.employees.filter(e =>
        ...
        (e.subdivision_id === subdivisionId || (e.subdivision_id === null && subdivisionId === undefined)) &&
...
function expandAll() {
    ...
    const allSubdivisions = props.structure.flatMap(d => d.subdivisions.map(s => s.id));
...
function openSectionModal(section: Section, divisionId: number, subdivisionId?: number) {
...
function getEmployeeCount(
    ...
    const count = props.employeeCounts.find(
        c =>
            ...
            (c.subdivision_id === subdivisionId || (c.subdivision_id === null && subdivisionId === undefined)) &&
...
function getSubdivisionEmployeeCount(subdivision: Subdivision, divisionId: number): number {
    return props.employeeCounts
        .filter(c => c.division_id === divisionId && c.subdivision_id === subdivision.id)
        .reduce((sum, c) => sum + c.count, 0);
}
...
const filteredStructure = computed(() => {
    ...
    if (searchQuery.value.trim()) {
        ...
        result = result.filter(division => {
            ...
            return division.subdivisions.some(sub => {
...
const totalSections = computed(() =>
    props.structure.reduce(
        (sum, d) =>
            sum +
            d.subdivisions.reduce((subSum, s) => subSum + s.sections.length, 0) +
            d.sections.length,
        0
    )
);
            ...
            <div class="grid grid-cols-2 gap-4 sm:grid-cols-4">
                <Card class="border border-gray-200 dark:border-neutral-800">
                    <CardHeader class="pb-2">
                        <CardTitle class="flex items-center gap-2 text-sm font-normal text-gray-500 dark:text-gray-400">
                    ...
                    <div v-else-if="viewMode === 'tree'" class="space-y-4">
                        ...
                        >
                                ...
                                >
                                    <div class="flex items-center gap-3">
                                        <div
                                            ...
                                            :class="{ 'rotate-180': expandedDivisions.has(division.id) }"
                                        >
                                ...
                                <div
                                    ...
                                    :class="expandedDivisions.has(division.id) ? 'max-h-[5000px] opacity-100' : 'max-h-0 opacity-0'"
                                >
                                    <div class="p-4">
                                        ...
                                        >
                                            ...
                                            <div class="flex flex-wrap gap-2">
                                                <div
                                                    ...
                                                    @click="openSectionModal(section, division.id)"
                                                >
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <div
                                                    ...
                                                    @click="subdivision.sections.length > 0 ? toggleSubdivision(subdivision.id) : null"
                                                >
                                                    <div class="flex items-center gap-3">
                                                        <div
                                                            ...
                                                            :class="{ 'rotate-180': expandedSubdivisions.has(subdivision.id) }"
                                                        >
                                                            <ChevronDown class="h-3.5 w-3.5 text-gray-500" />
                                                        </div>
                                                        <span v-else class="w-5" />
                                                        <div class="flex items-center gap-2">
                                                            <FolderTree class="h-4 w-4 text-gray-500" />
                                                            <span
                                                ...
                                                <div
                                                    class="overflow-hidden transition-all duration-300 ease-in-out"
                                                    :class="expandedSubdivisions.has(subdivision.id) && subdivision.sections.length > 0 ? 'max-h-[2000px] opacity-100' : 'max-h-0 opacity-0'"
                                                >
                                                    <div class="border-t border-gray-200 p-3 dark:border-neutral-700">
                                                        <div class="flex flex-wrap gap-2">
                                                            <div
                                                                ...
                                                                @click="openSectionModal(section, division.id, subdivision.id)"
                                                            >
                                                                <Network class="h-3.5 w-3.5 text-gray-400" />
                                                                <span
                                                                    class="text-sm text-gray-700 dark:text-gray-300"
                                                                >
                                                                    {{ section.name }}
                    ...
                    <div v-else-if="viewMode === 'hierarchy'" class="space-y-6">
                        ...
                        >
                            ...
                            <div
                                ...
                                :class="{ 'border-amber-500 bg-amber-50 dark:bg-amber-900/20': matchesSearch(division.name) }"
                            >
                            ...
                            <div class="relative ml-8 mt-2">
                                <div class="absolute left-0 top-0 bottom-0 w-0.5 bg-gray-300 dark:bg-neutral-600" />
                                ...
                                >
                                    <div
                                        ...
                                        class="relative"
                                    >
                                        <div class="absolute left-[-24px] top-3 h-0.5 w-6 bg-gray-300 dark:bg-neutral-600" />
                                        <div
                                            ...
                                            :class="{ 'border-amber-300 bg-amber-50 hover:bg-amber-100 dark:border-amber-700 dark:bg-amber-900/20 dark:hover:bg-amber-900/30': matchesSearch(section.name) }"
                                            @click="openSectionModal(section, division.id)"
                                        >
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center gap-2">
                                                ...
                                                <Badge variant="outline" class="text-xs">
                                                    {{ getEmployeeCount(division.id, undefined, section.id) }}
                                                </Badge>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                >
                                    <div
                                        v-for="(subdivision, idx) in division.subdivisions"
                                        :key="subdivision.id"
                                        class="relative pl-6"
                                    >
                                        <div
                                            class="absolute left-0 top-4 h-0.5 w-6 bg-gray-300 dark:bg-neutral-600"
                                            :class="{ 'bottom-0': idx === division.subdivisions.length - 1 && subdivision.sections.length === 0 }"
                                        />

                                        <!-- Subdivision -->
                                        <div
                                            class="rounded-md border border-gray-300 bg-white p-3 dark:border-neutral-600 dark:bg-neutral-800"
                                            :class="{ 'border-amber-400 bg-amber-50 dark:border-amber-600 dark:bg-amber-900/20': matchesSearch(subdivision.name) }"
                                        >
                                            <div class="flex items-center justify-between">
                                                <div class="flex items-center gap-2">
                                                    <FolderTree class="h-4 w-4 text-gray-500" />
                                                ...
                                                <Badge variant="secondary" class="text-xs">
                                                    {{ getSubdivisionEmployeeCount(subdivision, division.id) }}
                                                </Badge>
                                            </div>
                                        </div>
                                        ...
                                        >
                                            <div class="absolute left-0 top-0 bottom-3 w-0.5 bg-gray-200 dark:bg-neutral-700" />
                                            <div
                                                v-for="section in subdivision.sections"
                                                :key="section.id"
                                                class="relative"
                                            >
                                                <div
                                                    class="absolute left-[-24px] top-3 h-0.5 w-6 bg-gray-200 dark:bg-neutral-700"
                                                />
                                                <div
                                                ...
                                                >
                                                    <div class="flex items-center justify-between">
                                                        ...
                                                        <span class="text-xs text-gray-500">
                                                            {{ getEmployeeCount(division.id, subdivision.id, section.id) }}
                                                        </span>
                                                    </div>
                                                </div>
                                            </div>
                ...
                >
                    <div class="flex min-w-max flex-col items-center p-8">
                        ...
                        <div class="relative flex flex-col items-center">
                            <div class="z-10 rounded-xl border-2 border-brand bg-white p-5 text-center shadow-md dark:border-brand-dark dark:bg-neutral-800" :class="{ 'ring-4 ring-amber-200 dark:ring-amber-900/40': matchesSearch(division.name) }">
                                <Building2 class="mx-auto mb-2 h-8 w-8 text-brand" />
                                <h3 class="w-48 text-balance font-bold text-gray-900 dark:text-gray-100">{{ division.name }}</h3>
                                <Badge class="mt-2 text-xs">{{ getDivisionEmployeeCount(division) }} Members</Badge>
                            </div>

                            <!-- Connector line to Subdivisions -->
                            <div v-if="division.subdivisions.length > 0 || division.sections.length > 0" class="h-8 w-0.5 bg-gray-300 dark:bg-neutral-600"></div>
                        </div>

                        <!-- Children Container -->
                        <div v-if="division.subdivisions.length > 0 || division.sections.length > 0" class="relative flex justify-center gap-8 pt-4">
                            <!-- Horizontal distribution line built with borders -->
                            <div class="absolute left-1/2 top-0 h-4 border-t-2 border-l-2 border-gray-300 dark:border-neutral-600" :style="{ width: `calc(50% - 1rem)`, borderLeftWidth: (division.subdivisions.length > 1 || division.sections.length > 0) ? '2px' : '0' }"></div>
                            <div class="absolute right-1/2 top-0 h-4 border-t-2 border-r-2 border-gray-300 dark:border-neutral-600" :style="{ width: `calc(50% - 1rem)`, borderRightWidth: (division.subdivisions.length > 1 || division.sections.length > 0) ? '2px' : '0' }"></div>

                            <!-- Direct Sections -->
                            <div v-for="section in division.sections" :key="'sec-'+section.id" class="relative flex flex-col items-center">
                                <div class="absolute -top-4 h-4 w-0.5 bg-gray-300 dark:bg-neutral-600"></div>
                                <div
                                    ...
                                    :class="{ 'border-amber-300 bg-amber-50 dark:border-amber-700 dark:bg-amber-900/30': matchesSearch(section.name) }"
                                    @click="openSectionModal(section, division.id)"
                                >
                                    <Network class="mx-auto mb-1.5 h-5 w-5 text-gray-400" />
                                    <p class="text-sm font-semibold text-gray-700 dark:text-gray-300">{{ section.name }}</p>
                                    <p class="mt-1 text-xs text-gray-500">{{ getEmployeeCount(division.id, undefined, section.id) }} emp.</p>
                                </div>
                            </div>

                            <!-- Subdivisions -->
                            <div v-for="subdivision in division.subdivisions" :key="'sub-'+subdivision.id" class="relative flex flex-col items-center">
                                <div class="absolute -top-4 h-4 w-0.5 bg-gray-300 dark:bg-neutral-600"></div>

                                <!-- Subdivision Node -->
                                <div class="z-10 w-48 rounded-lg border border-gray-300 bg-white p-4 text-center shadow-sm dark:border-neutral-600 dark:bg-neutral-800" :class="{ 'ring-2 ring-amber-200 dark:ring-amber-900/40': matchesSearch(subdivision.name) }">
                                    <FolderTree class="mx-auto mb-2 h-6 w-6 text-gray-500" />
                                    <h4 class="font-medium text-gray-800 dark:text-gray-200">{{ subdivision.name }}</h4>
                                    <Badge variant="secondary" class="mt-2 text-[10px]">{{ getSubdivisionEmployeeCount(subdivision, division.id) }} Members</Badge>
                                </div>

                                <!-- Subdivision Sections -->
                                <div v-if="subdivision.sections.length > 0" class="flex flex-col items-center">
                                    <div class="h-6 w-0.5 bg-gray-200 dark:bg-neutral-700"></div>
                                    <div class="flex flex-col gap-3">
                                        <div
                                            v-for="section in subdivision.sections" :key="section.id"
                                            ...
                                            @click="openSectionModal(section, division.id, subdivision.id)"
                                        >
                                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">{{ section.name }}</p>
                                            <p class="text-[10px] text-gray-500">{{ getEmployeeCount(division.id, subdivision.id, section.id) }} employees</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <Dialog :open="sectionModalOpen" @update:open="closeSectionModal">
            <DialogContent class="max-w-2xl">
                <DialogHeader>
                    ...
                    <DialogDescription>
                        Employees in this section ({{ sectionEmployees.length }})
                    </DialogDescription>
                </DialogHeader>
                ...
                <div class="mt-4">
                    ...
                    <div v-else class="max-h-[400px] space-y-2 overflow-y-auto">
                        ...
                        >
                            <Avatar class="h-10 w-10 shrink-0 overflow-hidden rounded-full">
                                <AvatarImage
                                    v-if="typeof employee.avatar === 'string' && employee.avatar.trim() !== ''"
                                    :src="employee.avatar"
                                    :alt="getEmployeeFullName(employee)"
                                />
                                <AvatarFallback class="rounded-full bg-foreground text-background font-semibold">

...
watch([filterType, filterStatus], () => {
    ...
    filterDebounce = setTimeout(() => {
        ...
        router.get(employee.leaveApplications.index.url(), query, {
            preserveState: true,
        });
    }, 300);
});
...
watch(
    ...
    (app) => {
        if (app) {
            ...
            editDateFrom.value = app.date_from
                ? String(app.date_from).slice(0, 10)
                : '';
...
function submitAdd(e: Event) {
    e.preventDefault();
    router.post(
        employee.leaveApplications.store.url(),
        {
            type: addType.value,
...
function submitEdit(e: Event) {
    ...
    router.put(
        employee.leaveApplications.update.url(editingApp.value.id),
        {
            type: editType.value,
...
function inclusiveDates(app: Application) {
    const from = app.date_from
        ? formatDate(String(app.date_from).slice(0, 10))
        : '—';
    const to = app.date_to
        ? formatDate(String(app.date_to).slice(0, 10))
        : null;
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
            ...
            <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <Card class="border border-gray-200 dark:border-neutral-800">
                    ...
                    <CardContent>
                        ...
                        <div
                            v-if="props.leaveCredits.length"
                ...
                <div class="max-h-[60vh] space-y-3 overflow-y-auto p-1">
                    <dl class="grid grid-cols-1 gap-2 text-sm">
                        ...
                        <div>
                            ...
                            <dd class="mt-0.5">
                                <Badge
                                    :variant="statusVariant(viewingApp.status)"
                                >
                                    {{
                                        statusOptions[viewingApp.status] ??
                                        viewingApp.status
                                    }}
                                </Badge>
                            </dd>
                        ...
                        <div
                            v-if="
                                normalizeAttachments(viewingApp.attachments)
                                    .length
                            "
                        >
                            <dt
                        ...
                        <div>
                            ...
                            <dd class="mt-0.5">
                                {{ formatDate(viewingApp.created_at) }}
                            </dd>
                        </div>
                    </dl>
                </div>
        ...
        <Dialog v-model:open="cancelModalOpen">
            ...
            >
                ...
                <DialogFooter>
                    <Button type="button" variant="outline" @click="closeCancel"
                        >Keep</Button
                    >
                    <Button
                        type="button"
